...
function main() {
  ...
  process.stdout.write(`plugin component candidates: ${summary.total || 0}` + "\n");
  process.stdout.write(`providers: ${JSON.stringify(summary.byProvider || {})}` + "\n");
  process.stdout.write(`assetKinds: ${JSON.stringify(summary.byAssetKind || {})}` + "\n");
  process.stdout.write(`mode: ${inventory.mode || "offline-local"}` + "\n");
  if (args.learnStructure) {
    process.stdout.write(`learnedStructures: ${summary.learnedStructures || 0}` + "\n");
    process.stdout.write(`structureSignatures: ${summary.structureSignatures || 0}` + "\n");
  }
