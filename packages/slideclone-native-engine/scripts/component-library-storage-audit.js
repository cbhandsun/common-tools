...
function main() {
  ...
  process.stdout.write(`files: ${plan.summary.files}` + "\n");
  process.stdout.write(`exact duplicate reclaimable MiB: ${(plan.summary.exactDuplicateReclaimableBytes / 1024 / 1024).toFixed(2)}` + "\n");
  process.stdout.write(`regenerable evidence MiB: ${(plan.summary.regenerableEvidenceBytes / 1024 / 1024).toFixed(2)}` + "\n");
  process.stdout.write(`storage audit: ${path.resolve(args.out)}` + "\n");
}
