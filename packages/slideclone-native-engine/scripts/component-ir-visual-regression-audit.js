...
function sanitizeErrorMessage(error) {
  return stripControlCharacters(String(error?.message || error || "unknown error"), " ").slice(0, 1000);
}

function stripControlCharacters(text, replacement = "") {
  let output = "";
  for (let index = 0; index < text.length; index += 1) {
    const code = text.charCodeAt(index);
    output += code > 0x1F && code !== 0x7F ? text[index] : replacement;
  }
  return output;
}
