...
const paddleOcrLocalAdapter = require("./adapters/ocr-paddleocr-local");
...
async function maybeStartPaddleOcrBroker({ args = {}, textOcr = true, textOcrAdapter = DEFAULT_OCR_ADAPTER, cases = [], concurrency = 1 } = {}) {
  ...
  const expectedAdapterFile = path.resolve(__dirname, "adapters", "ocr-paddleocr-local.js");
  if (path.resolve(adapterFile) !== expectedAdapterFile) throw new Error("PaddleOCR batch adapter must be the packaged local adapter");
  return startPaddleOcrBatchBroker({
    adapter: paddleOcrLocalAdapter,
    context: {
