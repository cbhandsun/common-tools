...
function main() {
  ...
  process.stdout.write(`component library refresh: ${result.mode}\n`);
  for (const entry of result.results) {
    process.stdout.write(`- ${entry.name}: ${entry.status}\n`);
  }
  process.stdout.write(`motif report: ${path.resolve(result.paths.motifRecall)}\n`);
  process.stdout.write(`action queue: ${path.resolve(result.paths.actionQueue)}\n`);
  process.stdout.write(`action guide: ${path.resolve(result.paths.actionQueueGuide)}\n`);
}
