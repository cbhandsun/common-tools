...
function safeMatrixText(value) {
  return stripControlCharacters(String(value ?? "")).trim().slice(0, 180);
}
...
function stripControlCharacters(text) {
  ...
  for (let index = 0; index < text.length; index += 1) {
    ...
    if (code > 0x1F && code !== 0x7F) output += text[index];
  }
  return output;
}
...
function main() {
  ...
  process.stdout.write(`${JSON.stringify({ ...matrix, reportFile: outFile }, null, 2)}\n`);
