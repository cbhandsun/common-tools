...
function createFoundationCapabilityNetworkObjects(images = [], textBoxes = [], slideSize = DEFAULT_SLIDE) {
...
function standardizedFourLayerBlockShapes(sourceId, layer, index) {
...
function cellBox(table, row, col) {
  return {
    x: round(table.cols[col]),
    y: round(table.rows[row]),
    w: round(table.cols[col + 1] - table.cols[col]),
...
function temporaryAnswerWorkflowMatrixTextBoxes(table, source) {
  const rows = temporaryAnswerWorkflowMatrixRows();
  const textBoxes = [];
  rows.forEach((row, rowIndex) => {
    row.forEach((text, colIndex) => {
      const box = cellBox(table, rowIndex, colIndex);
      const isHeader = rowIndex === 0;
      const isLeft = colIndex === 0;
      const portalHeader = isHeader && colIndex === 3;
      textBoxes.push(temporaryAnswerWorkflowTextBox(
        `temporary-answer-workflow-text-${rowIndex}-${colIndex}`,
        text,
        {
          x: round(box.x + (portalHeader ? 65 : (isLeft ? 8 : 18))),
          y: round(box.y + (isHeader ? 7 : 12)),
          w: round(box.w - (portalHeader ? 72 : (isLeft ? 16 : 28))),
          h: round(box.h - (isHeader ? 14 : 18))
        },
        { sizePt: isHeader ? 17 : (isLeft ? 17 : 16), color: isHeader ? "#FFFFFF" : "#111111", weight: isHeader || isLeft || colIndex === 3 ? "bold" : "regular", align: isLeft || isHeader ? "center" : "left" },
        source("temporary-answer-workflow-native-text", { row: rowIndex, col: colIndex })
      ));
    });
  });
  textBoxes.push(temporaryAnswerWorkflowMatrixQuoteTextBox(table, source));
  return textBoxes;
}
