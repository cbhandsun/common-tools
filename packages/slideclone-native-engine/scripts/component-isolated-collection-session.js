...
function buildCollectionGuide({ provider, fixturePptx, outDir }) {
  return [
    ...
    "",
    "```powershell",
    `node packages\\slideclone-native-engine\\scripts\\component-isolated-collection-session.js --ingest "${fixturePptx}" --provider ${provider} --label <component-name> --out "${outDir}"`,
    "```",
    "",
    "```powershell",
    `node packages\\slideclone-native-engine\\scripts\\component-asset-self-fidelity-batch.js --root "${path.join(outDir, "verified", provider)}" --out "${path.join(outDir, "self-fidelity", provider)}" --concurrency 2 --fail-on-reject`,
    "```",
    ""
  ].join("\n");
}
...
function sanitizeLabel(value) {
  const label = stripControlCharacters(String(value || ""))
    .trim()
