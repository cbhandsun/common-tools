...
function renderActionGuide({ queueFile, actions, watchProvider, durationMs, activePowerPoint = false }) {
  const lines = [
    ...
    "",
    "```powershell",
    "node packages\\slideclone-native-engine\\scripts\\harvest-active-powerpoint-component.js --provider <officeplus|islide> --label <candidate-id-or-short-name>",
    "```",
    ""
  ];
...
function activeSlideHarvestCommand(action = {}) {
  ...
  return `node packages\\slideclone-native-engine\\scripts\\harvest-active-powerpoint-component.js --provider ${provider} --label ${label}`;
}
...
async function main() {
  ...
  process.stdout.write([
    `plugin apply session actions: ${report.actions.length}`,
    `changed plugin component files: ${report.watch?.changedCount ?? 0}`,
    `guide: ${report.guide}`,
    `report: ${path.join(report.outDir, "plugin-apply-session.json")}`
  ].join("\n") + "\n");
}
