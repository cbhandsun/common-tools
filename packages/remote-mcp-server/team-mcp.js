...
function teamToolProperties(name, capabilities = Object.keys(CAPABILITY_SCOPES), requireProjectRbac = false) {
  ...
  const byTool = {
    ...
    create_team_job: ["capability", "inputObjectKey", "idempotencyKey"],
    get_team_job: ["id"],
    cancel_team_job: ["id"],
...
async function callTeamTool(name, rawArgs, { principal, services, now = () => Date.now(), requireProjectRbac = false, enabledCapabilities: rawEnabledCapabilities, traceParent }) {
  ...
  if (name === "create_team_job") {
    const args = teamArgs(rawArgs, ["capability", "inputObjectKey", "idempotencyKey"], ["capability", "inputObjectKey", "idempotencyKey"], requireProjectRbac);
    const capability = authorizedCapability(principal, args.capability, enabledCapabilities);
    if (requireProjectRbac) projectAccess(principal, args.projectId, ["editor", "admin"]);
    return validateTeamToolOutput(name, await services.createJob({ capability, ownerId: principal.subject, projectId: requireProjectRbac ? args.projectId : undefined, inputObjectKey: args.inputObjectKey, idempotencyKey: args.idempotencyKey, expiresAt: new Date(now() + 24 * 60 * 60 * 1000).toISOString(), traceParent }));
  }
