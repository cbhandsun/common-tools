...
function canonicalizeIr(value, irDir, fileMemo = new Map()) {
  if (Array.isArray(value)) return value.map((item) => canonicalizeIr(item, irDir, fileMemo));
  if (!value || typeof value !== "object") return value;
  const output = {};
  for (const name of Object.keys(value).sort()) {
    const item = value[name];
    // Match the builder's case-insensitive JSON model binding.
    const field = name.toLowerCase();
    if ((field === "assetpath" || field === "sourceimage") && typeof item === "string" && item) {
      ...
      output[name] = canonicalizeIr(item, irDir, fileMemo);
    }
  }
  return output;
}
...
function copyAtomic(source, destination) {
  ...
  try {
    ...
    try { fs.rmSync(temp, { force: true }); } catch {
      // Preserve the copy/publish error; callers already treat unavailable cache as a miss.
    }
    throw error;
  }
}
