...
const ARROW_SIZES = new Set(["small", "medium", "large"]);
...
function arrowSize(value, label) {
  const normalized = value === undefined ? "medium" : String(value).trim().toLowerCase();
  if (!ARROW_SIZES.has(normalized)) throw new TypeError(`${label} is invalid`);
  return normalized;
}
...
function resolveConnectorComponent(input) {
  ...
  const arrowWidth = arrowSize(input.arrowWidth, "connector arrow width");
  const arrowLength = arrowSize(input.arrowLength, "connector arrow length");
  ...
  return {
    ...
    style: {
      ...
      ...(direction === "bidirectional" ? {
        startArrowWidth: arrowWidth, startArrowLength: arrowLength,
        endArrowWidth: arrowWidth, endArrowLength: arrowLength
      } : direction === "forward" ? { endArrowWidth: arrowWidth, endArrowLength: arrowLength } : {}),
    },
