...
const { buildComponentRegions, compareComponentRegions } = require("./component-region-quality");
  ...
  const jobs = pages.map((page) => {
    ...
    return {
      ...
      options: {
        ...
        componentRegions: input.ir?.slideSize ? buildComponentRegions(page, input.ir.slideSize) : [],
...
function comparePageInWorker(job) {
  return new Promise((resolve, reject) => {
    const worker = new Worker(path.join(__dirname, "pixel-diff-page-worker.js"), { workerData: job });
...
function compareImages(pageIndex, source, generated, options, renderedImage, diffImage) {
  ...
  const componentQuality = options.slideSize && Array.isArray(options.componentRegions)
    ? compareComponentRegions(source, generated, options.componentRegions, options.slideSize, options)
    : null;
  return {
    pageIndex,
    ok: true,
