...
/** @typedef {{ width: number, height: number, rgba: Buffer }} PngImage */
/** @typedef {{ x: number, y: number, w: number, h: number }} PngCropBox */
/** @typedef {{ label?: string, maxFileBytes?: unknown, maxPixels?: unknown, maxDimension?: unknown, maxInflatedBytes?: unknown }} PngReadOptions */
/** @typedef {{ maxFileBytes: number, maxPixels: number, maxDimension: number, maxInflatedBytes: number }} PngReadLimits */
/** @typedef {{ maxBytes?: unknown }} PngCacheOptions */
/** @typedef {{ hits: number, misses: number, entries: Map<string, PngImage>, bytes: number, maxBytes: number, stats: () => { hits: number, misses: number, entries: number, bytes: number } }} PngReadCache */
...
/** @type {import("node:async_hooks").AsyncLocalStorage<PngReadCache>} */
...
function readPng(file, options = {}) {
  ...
  if (cached) {
    ...
    /** @type {PngReadCache} */ (activeCache).hits += 1;
...
/** @param {PngImage} image @param {import("node:fs").Stats} stats @param {PngReadLimits} limits @param {string} file */
...
/** @param {PngImage} image @returns {PngImage} */
...
function readPngBuffer(buffer, options = {}) {
  ...
  while (offset + 12 <= buffer.length) {
    ...
    if (type === "IHDR") {
      ...
      bitDepth = /** @type {number} */ (data[8]);
      colorType = /** @type {number} */ (data[9]);
  ...
  for (let y = 0; y < height; y += 1) {
    const filter = /** @type {number} */ (raw[rawOffset++]);
...
/** @param {string} file @param {PngImage} image */
...
/** @param {PngImage} source @param {PngCropBox} box @returns {PngImage} */
...
/** @param {number} colorType @param {string} file @returns {number} */
...
/** @param {Buffer} line @param {Buffer} previous @param {number} filter @param {number} bpp */
function unfilter(line, previous, filter, bpp) {
  for (let i = 0; i < line.length; i += 1) {
    const left = i >= bpp ? /** @type {number} */ (line[i - bpp]) : 0;
    const up = previous[i] || 0;
    const upLeft = i >= bpp ? previous[i - bpp] || 0 : 0;
    const current = /** @type {number} */ (line[i]);
    if (filter === 1) line[i] = (current + left) & 0xFF;
    else if (filter === 2) line[i] = (current + up) & 0xFF;
    else if (filter === 3) line[i] = (current + Math.floor((left + up) / 2)) & 0xFF;
    else if (filter === 4) line[i] = (current + paeth(left, up, upLeft)) & 0xFF;
...
/** @param {Buffer} line @param {Buffer} rgba @param {number} y @param {number} width @param {number} channels @param {number} colorType @param {Buffer | null} [palette] @param {Buffer | null} [paletteAlpha] */
function writeRgba(line, rgba, y, width, channels, colorType, palette = null, paletteAlpha = null) {
  for (let x = 0; x < width; x += 1) {
    const src = x * channels;
    const dst = (y * width + x) * 4;
    if (colorType === 0) {
      rgba[dst] = /** @type {number} */ (line[src]);
      rgba[dst + 1] = /** @type {number} */ (line[src]);
      rgba[dst + 2] = /** @type {number} */ (line[src]);
      rgba[dst + 3] = 255;
    } else if (colorType === 3) {
      const index = /** @type {number} */ (line[src]);
      const paletteOffset = index * 3;
      if (!palette || paletteOffset + 2 >= palette.length) {
        throw new Error(`PNG palette index ${index} is out of bounds.`);
      }
      rgba[dst] = /** @type {number} */ (palette[paletteOffset]);
      rgba[dst + 1] = /** @type {number} */ (palette[paletteOffset + 1]);
      rgba[dst + 2] = /** @type {number} */ (palette[paletteOffset + 2]);
      rgba[dst + 3] = paletteAlpha && index < paletteAlpha.length ? /** @type {number} */ (paletteAlpha[index]) : 255;
    } else if (colorType === 2) {
      rgba[dst] = /** @type {number} */ (line[src]);
      rgba[dst + 1] = /** @type {number} */ (line[src + 1]);
      rgba[dst + 2] = /** @type {number} */ (line[src + 2]);
      rgba[dst + 3] = 255;
    } else if (colorType === 4) {
      rgba[dst] = /** @type {number} */ (line[src]);
      rgba[dst + 1] = /** @type {number} */ (line[src]);
      rgba[dst + 2] = /** @type {number} */ (line[src]);
      rgba[dst + 3] = /** @type {number} */ (line[src + 1]);
    } else {
      rgba[dst] = /** @type {number} */ (line[src]);
      rgba[dst + 1] = /** @type {number} */ (line[src + 1]);
      rgba[dst + 2] = /** @type {number} */ (line[src + 2]);
      rgba[dst + 3] = /** @type {number} */ (line[src + 3]);
    }
  }
}

/** @param {number} width @param {number} height @param {number} bitDepth @param {number} colorType @returns {Buffer} */
...
/** @param {number} width @param {number} height @param {PngReadLimits} limits @param {string} file */
