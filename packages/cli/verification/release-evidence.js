...
function verifyReleaseEvidence({ manifestPath, packagePath = "package.json", lockPath = "package-lock.json", sbomPath, revision }) {
  const evidence = readReleaseEvidenceFile(manifestPath).evidence;
  if (revision !== undefined && assertRevision(revision) !== evidence.source.revision) throw new Error("release evidence does not match the expected revision");
...
function verifyReleaseEvidenceFile({ manifestPath, packagePath = "package.json", lockPath = "package-lock.json", revision }) {
  ...
  return verifyReleaseEvidence({ manifestPath: document.manifestPath, packagePath, lockPath, sbomPath, revision });
}
