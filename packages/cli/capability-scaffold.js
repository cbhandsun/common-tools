...
function draftManifest(name) {
  const packageName = `@common-tools/${name}-core`;
  return JSON.stringify({ status: "draft", capability: name, version: "0.1.0", toolNames: [], dependencies: [], minimumRuntimeVersion: ">=0.1.0 <1.0.0", requiredWorkerProfile: "base", team: { oauthScope: `common-tools:capability:${name}`, acceptedUploadMediaTypes: ["application/gzip"] }, moduleSource: { packageName, requirePath: `../${name}-core`, exportName: "CAPABILITY_MODULE" }, note: "This draft is intentionally not a runtime capability manifest. Declare any prerequisite capabilities explicitly, then complete a handler, worker profile, moduleSource owner, tests, and verified contentSha256 before moving it under packages/capability-manifests/." }, null, 2).concat("\n");
}
function readme(name) {
  return `# ${titleFor(name)} capability draft\n\nThis bundle contains self-contained Codex and Claude plugin packages plus mirrored local marketplaces. It is intentionally not registered with the common-tools Runtime.\n\n## Safe integration sequence\n\n1. Implement the owned capability package and export its \`CAPABILITY_MODULE\`; direct remote capabilities must export \`REMOTE_CAPABILITY_MODULE\` instead.\n2. Implement the local MCP handler and add its exact tool names, prerequisite capabilities, and \`moduleSource\` owner to a verified capability manifest.\n3. Implement a constrained team Worker when remote execution is required; do not add a capability to the team allowlist without a worker.\n4. For a remote capability, declare one bounded \`team.deployment\` block in its manifest and add the matching Compose Worker service/profile/image. Local-only capabilities must omit that block.\n5. Copy the host packages and marketplace mirrors into the common-tools repository.\n6. Run \`node scripts/generate-capability-catalogs.js\`, \`npm run common-tools:verify-capabilities\`, \`npm run common-tools:verify-plugins\`, and \`npm run common-tools:test\`.\n7. Publish the Runtime first, then install only this capability from the relevant marketplace.\n\nThe draft Skill must remain guidance-only until those steps are complete.\n`;
}
