...
function analysisCanvas(slide, index, theme, plan) {
  ...
  const cells = new Map(visual.entries.map((entry, entryIndex) => { const x = 56 + (entryIndex % columns) * (cellW + gap); const y = 194 + Math.floor(entryIndex / columns) * (cellH + gap); return [entry.id, { x, y, centerX: x + cellW / 2, centerY: y + cellH / 2 }]; }));
  for (const link of visual.links || []) { const from = cells.get(link.from); const to = cells.get(link.to); const left = Math.min(from.centerX, to.centerX); const top = Math.min(from.centerY, to.centerY); page.shapes.push(shape(`${slide.id}-${link.id}-connector`, "line", box(left, top, Math.max(0.01, Math.abs(to.centerX - from.centerX)), Math.max(0.01, Math.abs(to.centerY - from.centerY))), { stroke: theme.muted, strokeWidthPt: 1.5, endArrow: "triangle" })); }
  visual.entries.forEach((entry, entryIndex) => { const { x, y } = cells.get(entry.id); page.shapes.push(shape(`${slide.id}-${entry.id}-analysis`, "roundRect", box(x, y, cellW, cellH), { fill: entryIndex % 2 ? theme.surface : theme.primary, stroke: theme.line, strokeWidthPt: 1, radiusPt: 12 })); page.textBoxes.push(text(`${slide.id}-${entry.id}-label`, entry.label, box(x + 18, y + 18, cellW - 36, 34), { family: theme.font, sizePt: 20, weight: "bold", color: entryIndex % 2 ? theme.text : theme.inverse, align: "left" }, "analysis-label")); if (entry.detail) page.textBoxes.push(text(`${slide.id}-${entry.id}-detail`, entry.detail, box(x + 18, y + 58, cellW - 36, Math.max(34, cellH - 74)), { family: theme.font, sizePt: 16, color: entryIndex % 2 ? theme.muted : theme.inverse, align: "left" }, "analysis-detail")); }); return page;
}
