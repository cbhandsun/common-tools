...
const { planSemanticAnalysis } = require("./semantic-components");
...
function applyResolvedMedia(page, slide, assetPaths, assetInfo) {
  ...
  const sourceInfo = assetInfo?.[visual.assetId]?.source;
  if (sourceInfo?.attribution) page.textBoxes.push(text(`${slide.id}-asset-attribution`, sourceInfo.attribution, box(68, 490, 824, 20), { family: "Arial", sizePt: 9, color: "#64748B", align: "right" }, "asset-attribution"));
  return page;
}
...
function analysisCanvas(slide, index, theme, plan) {
  const page = pageBase(slide, index, theme, plan); const visual = slide.visual; addHeader(page, slide, theme, index); const semantic = planSemanticAnalysis(visual, box(56, 194, 848, 274)); const entries = new Map(visual.entries.map((entry) => [entry.id, entry]));
  for (const link of semantic.links) { const left = Math.min(link.from.x, link.to.x); const top = Math.min(link.from.y, link.to.y); page.shapes.push(shape(`${slide.id}-${link.id}-connector`, "line", box(left, top, Math.max(0.01, Math.abs(link.to.x - link.from.x)), Math.max(0.01, Math.abs(link.to.y - link.from.y))), { stroke: theme.muted, strokeWidthPt: 1.5, endArrow: "triangle" })); }
  semantic.nodes.forEach((node, entryIndex) => { const entry = entries.get(node.id); const bounds = node.box; page.shapes.push(shape(`${slide.id}-${entry.id}-analysis`, node.shape, bounds, { fill: entryIndex % 2 ? theme.surface : theme.primary, stroke: theme.line, strokeWidthPt: 1, radiusPt: 12 })); page.textBoxes.push(text(`${slide.id}-${entry.id}-label`, entry.label, box(bounds.x + 14, bounds.y + 12, bounds.w - 28, Math.min(34, bounds.h - 18)), { family: theme.font, sizePt: 18, weight: "bold", color: entryIndex % 2 ? theme.text : theme.inverse, align: "center" }, "analysis-label")); if (entry.detail && bounds.h >= 76) page.textBoxes.push(text(`${slide.id}-${entry.id}-detail`, entry.detail, box(bounds.x + 14, bounds.y + 48, bounds.w - 28, Math.max(20, bounds.h - 60)), { family: theme.font, sizePt: 14, color: entryIndex % 2 ? theme.muted : theme.inverse, align: "center" }, "analysis-detail")); });
  page.semanticComponents = [Object.freeze({ id: `${slide.id}-semantic-analysis`, kind: semantic.component, model: semantic.model, objectIds: Object.freeze(semantic.nodes.map((node) => node.id)) })]; return page;
}
function analysisSteps(slide, index, theme, plan) {
  const page = pageBase(slide, index, theme, plan); const visual = slide.visual; addHeader(page, slide, theme, index); const semantic = planSemanticAnalysis(visual, box(56, 198, 600, 264)); const entries = new Map(visual.entries.map((entry) => [entry.id, entry]));
  for (const link of semantic.links) { const left = Math.min(link.from.x, link.to.x); const top = Math.min(link.from.y, link.to.y); page.shapes.push(shape(`${slide.id}-${link.id}-connector`, "line", box(left, top, Math.max(0.01, Math.abs(link.to.x - link.from.x)), Math.max(0.01, Math.abs(link.to.y - link.from.y))), { stroke: theme.muted, strokeWidthPt: 1.5, endArrow: "triangle" })); }
  semantic.nodes.forEach((node, entryIndex) => { const entry = entries.get(node.id); page.shapes.push(shape(`${slide.id}-${entry.id}-node`, node.shape, node.box, { fill: entryIndex % 2 ? theme.surface : theme.primary, stroke: theme.line, strokeWidthPt: 1, radiusPt: 10 })); page.textBoxes.push(text(`${slide.id}-${entry.id}-label`, entry.label, box(node.box.x + 10, node.box.y + 8, node.box.w - 20, Math.max(18, Math.min(34, node.box.h - 16))), { family: theme.font, sizePt: 15, weight: "bold", color: entryIndex % 2 ? theme.text : theme.inverse, align: "center" }, "analysis-label")); });
  page.shapes.push(shape(`${slide.id}-analysis-insight-panel`, "roundRect", box(686, 198, 218, 264), { fill: theme.surface, stroke: theme.accent, strokeWidthPt: 2, radiusPt: 14 })); page.textBoxes.push(text(`${slide.id}-analysis-insight`, visual.insight || slide.items[0].label, box(708, 226, 174, 206), { family: theme.font, sizePt: 18, weight: "bold", color: theme.text, align: "left" }, "insight")); page.semanticComponents = [Object.freeze({ id: `${slide.id}-semantic-analysis`, kind: semantic.component, model: semantic.model, objectIds: Object.freeze(semantic.nodes.map((node) => node.id)) })]; return page;
}
