...
function checkedFile(file, maximum, label) {
...
function loadContentProviderConfig({ configFile, allowedRoot, fetchImpl = globalThis.fetch } = {}) {
  ...
  const file = checkedFile(resolveFile(configFile, root || process.cwd(), root, "content provider config"), MAX_CONFIG_BYTES, "content provider config");
  let value;
  ...
  const providers = value.providers.map((provider) => {
    ...
    const tokenFile = checkedFile(resolveFile(provider.tokenFile, path.dirname(file), root, "content provider token"), MAX_TOKEN_BYTES, "content provider token");
