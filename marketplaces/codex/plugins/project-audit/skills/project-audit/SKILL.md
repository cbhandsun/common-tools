...
Run `common-tools audit run --mode gates --scope <selected-scope-ids> --run-gates --out <output>` only after explicit authorization. Report each declared `check`, `lint`, `typecheck`, `test`, and `build` result separately. An unconfigured, unavailable, timed-out, or unrun gate is not a pass.
...
Create a separately reviewed manifest after inspection, then run `common-tools audit run --mode experience --scope <selected-scope-ids> --experience-evidence <reviewed-manifest.json> --out <output>`. Use `--mode full --scope <selected-scope-ids> --run-gates --experience-evidence <reviewed-manifest.json>` for a full audit.
