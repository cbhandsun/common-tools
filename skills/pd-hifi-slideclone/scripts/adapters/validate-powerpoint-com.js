...
async function validatePowerPointOpen(pptxFiles, options = {}) {
  ...
  for (let launchAttempt = 1; launchAttempt <= launchAttempts; launchAttempt += 1) {
    ...
    try {
      ...
      if (launchAttempt < launchAttempts && isRetryableColdStartReport(report)) {
        await wait(launchAttempt * 3000);
        continue;
      }
      const details = [error.stderr, error.stdout, error.message].filter(Boolean).join("\n").trim();
      throw new Error(`PowerPoint open gate failed: ${details || "unknown PowerPoint COM error"}`);
    }
  }
  ...
  if (report?.passed !== true) {
    const failures = Array.isArray(report?.results)
      ...
      : [];
    throw new Error(`PowerPoint open gate rejected ${failures.length || "one or more"} PPTX file(s): ${failures.join(", ")}`);
  }
  return report;
}
...
try {
  ...
  function Open-PresentationWithRetry([string]$FilePath) {
    for ($openAttempt = 1; $openAttempt -le 12; $openAttempt++) {
      ...
      Start-Sleep -Milliseconds (600 * $openAttempt)
    }
  ...
  function Get-PresentationModifiedWithRetry($Presentation) {
    for ($savedAttempt = 1; $savedAttempt -le 12; $savedAttempt++) {
      ...
      Start-Sleep -Milliseconds (250 * $savedAttempt)
    }
