...
test("local Keycloak realm mints the restricted project-membership claim without users or credentials", () => {
  ...
  assert.equal(realm.editUsernameAllowed, false);
  assert.equal(realm.rememberMe, false);
  assert.equal(realm.duplicateEmailsAllowed, false);
  assert.deepEqual(realm.identityProviders, []);
  assert.equal(realm.bruteForceProtected, true);
  assert.equal(realm.permanentLockout, false);
  assert.equal(realm.failureFactor, 5);
  assert.equal(realm.waitIncrementSeconds, 60);
  assert.equal(realm.quickLoginCheckMilliSeconds, 1000);
