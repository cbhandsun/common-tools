...
- native rebuild center badge quadrant cycle：中心徽章四象限循环页的 layout 推断、象限/箭头 native shapes、source-faithful 中心 crop、OCR 文字修复和颜色采样从主 rebuild 编排中移出到独立 factory；主入口只保留依赖装配和同名导出合同。
...
- native rebuild generic node/cluster：generic node skeleton、multi-cluster skeleton、portal four-layer visual cluster、visual cluster stack 的对象化判定、native shape/text、颜色/布局测量和 component metadata 从主 rebuild 编排中移出；主入口只保留工厂装配和同名导出合同。
- native rebuild collaboration/sticky diagrams：collaboration flow 的 hub/card/branch connector、semantic text、OCR erase 写回，以及 sticky note cluster 的颜色组件检测、便签 native shape 和 region 元数据从主 rebuild 编排中移出；主入口只注入像素、颜色、几何和 IR asset 边界。
- native rebuild structured illustration：process-with-screenshots 类结构插画的 card shell、title/processing warning、input/output document、input chaos glyph 与 residual line/icon/sketch/gear-person 重建从主 rebuild 编排中移出；落点为一个薄 facade 加 cards/residuals 两个子 factory，避免形成新的大模块。
...
- native rebuild dense text grid + layer visual primitives：dense text grid/table stroke 推断先抽为独立公共边界，供 grid/color factory 消费；layer/matrix color blocks、table-grid 准入、layer container/connector 像素采样与对象化迁入 layer visual primitives，主入口只保留工厂装配和同名导出合同。
- native rebuild residual crop lifecycle：layer text crop 擦除、erased residual split/drop decision、table/matrix residual native replacement、tool-gap residual text erase 与 residual split 判定从主 rebuild 编排中移出；主入口只注入 drop policy、PNG/IR、像素擦除和保护规则。
...
- 后续增量 `native-rebuild-center-badge-quadrant-cycle.js` 抽出后，`rebuild-real-pptx-native.js` 从 12,426 行降至 12,063 行；新增模块 446 行，`native-rebuild.test.js`、lint、runtime package、workspace boundaries、architecture budgets 与 closeout 汇总均通过。
...
- 后续增量 `native-rebuild-generic-node-cluster.js` 抽出后，`rebuild-real-pptx-native.js` 从 9,465 行降至 8,693 行；新增模块 1,481 行，`native-rebuild.test.js`、lint、runtime package、workspace boundaries、architecture budgets 与 closeout 汇总均通过。
- 后续增量 `native-rebuild-collaboration-sticky-diagrams.js` 抽出后，`rebuild-real-pptx-native.js` 从 8,693 行降至 8,025 行；新增模块 1,118 行，`native-rebuild.test.js`、lint、runtime package、workspace boundaries、architecture budgets 与 closeout 汇总均通过。
- 后续增量 `native-rebuild-structured-illustration.js` facade、`native-rebuild-structured-illustration-cards.js` 与 `native-rebuild-structured-illustration-residuals.js` 抽出后，`rebuild-real-pptx-native.js` 从 8,025 行降至 6,828 行；子模块分别为 13/1,417/788 行，`native-rebuild.test.js`、lint、runtime package、workspace boundaries、architecture budgets 与 closeout 汇总均通过。
...
- 后续增量 `native-rebuild-dense-text-grid.js` 与 `native-rebuild-layer-visual-primitives.js` 抽出后，`rebuild-real-pptx-native.js` 从 5,084 行降至 4,560 行；新增模块分别为 99/523 行，`native-rebuild.test.js`、lint、runtime package、workspace boundaries、architecture budgets 与 closeout 汇总均通过。
- 后续增量 `native-rebuild-residual-crop-lifecycle.js` 抽出后，`rebuild-real-pptx-native.js` 从 4,560 行降至 4,005 行；新增模块 618 行，`native-rebuild.test.js`、lint、runtime package、workspace boundaries、architecture budgets 与 closeout 汇总均通过。
...
- 本批新增 `team-runtime-local-acceptance.ps1` / `npm run common-tools:team-local-acceptance`，把本地部署、Keycloak、测试用户和 authenticated Job smoke 串成单入口；默认只提示一次共享本地验收密码并在当前进程临时复用，脚本结束后恢复原环境变量。成功时写入 `artifacts/local-acceptance/` 脱敏 JSON 证据并自动调用 `verify-local-acceptance-evidence.js` 复核通过条件和脱敏边界；后续又补了 `-PreflightOnly`，可在不部署、不打开浏览器、不输入密码的情况下先输出会使用的 evidence 路径、是否会部署/登录以及当前缺哪些本机 secret；需要排障时仍可拆成三条子命令。
- 本批新增 `team-runtime-local-closeout.ps1` / `npm run common-tools:team-local-closeout`，把 fresh reset、受控本地部署、authenticated acceptance evidence 和 architecture closeout 串成最终一键入口；用户只输入一次不少于 8 位的共享本地密码，脚本会临时同步 PostgreSQL、应用数据库连接、Redis、MinIO、Keycloak admin 和本机测试用户密码，结束后恢复当前进程环境变量。
...
后续增量 `514e86c Add local authenticated job smoke wrapper` 新增 `.\scripts\team-runtime-local-job-smoke.ps1` / `npm run common-tools:team-local-job-smoke`。该入口会按能力自动生成最小输入：`image-to-editable` 打包 team raw-image archive，`ppt-create` 打包 hash-bound `ppt-create` archive，`ppt-quality` / `ppt-improve` 生成受限最小 PPTX，其中 `ppt-improve` 默认带 `safe-package` repair profile，然后发现本地 gateway 并调用受保护 MCP 的 authenticated Job smoke。默认仍可通过 `COMMON_TOOLS_JOB_SMOKE_TOKEN` 显式提供 OAuth bearer token；若本地 Keycloak 已运行，也可传 `-Login`，脚本会打开浏览器走 Authorization Code + PKCE S256，loopback 收到 code 后只把 access token 临时放入当前 PowerShell 进程再调用 smoke，不保存、不打印 token。默认不等待 Worker 终态；传 `-Wait` 时会按能力验证默认 artifact target：`image-to-editable` / `ppt-create` 为 `deck.pptx`，`ppt-quality` 为 `ppt-quality-report.json`，`ppt-improve` 为 `ppt-improve-report.json`。
