...
The first successful immutable commercial-release candidate, `v0.1.15`, is published. Core engineering is Beta-ready; commercial release remains blocked until a production deployment and public end-to-end canaries are complete. The immutable `v0.1.14` tag is retained as failed release evidence: its workflow stopped before image build because the hosted Linux npm CLI layout was not supported, and no GitHub Release was created.
...
| P0 | First immutable release | done | Tag matches `package.json`; release workflow succeeds once; SBOM, signed evidence, GHCR digests, Cosign verification, attestations, and GitHub Release are present | [v0.1.15 Release](https://github.com/cbhandsun/common-tools/releases/tag/v0.1.15), run `33300262845`, failed `v0.1.14` run `33293402721` retained | Release engineering | v0.1.15 |
| P0 | Marketplace immutable ref | in progress | installation guide and validated Marketplace configuration reference the released tag rather than `main` | `docs/git-marketplace-installation.md`, `v0.1.15`; clean-machine install report pending | Product release | v0.1.15 |
...
1. Done: land the stable Office required check and make it a required `main` check together with CI.
2. Done: create and verify `v0.1.15`; record all immutable digests and release evidence while retaining the failed `v0.1.14` run as evidence.
