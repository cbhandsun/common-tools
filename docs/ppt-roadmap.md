...
### 后续改动：串行语料复用 PowerPoint（正式验收通过，待合入）
...
真实双案例集成中两例均通过，分别约 12.7 秒和 4.3 秒，第二例 COM 附着 36 毫秒、终结器 49 毫秒；统一关闭返回时 POWERPNT 进程数为 0。首轮 full `33457631577` 的 corpus 31/31 通过，但后置独立编辑往返第 5/5 个在关闭时收到瞬时 `0x80010001`；后续 smoke `33460946205` 又暴露首个 deck 的可编辑集合在原 8 次等待窗口内未就绪。修复只对两个明确的瞬时 RPC HRESULT 做最多 5 次关闭退避，并把只读目标发现窗口扩为 12 次；永久错误仍立即失败。文档候选 `8c53768` 的 smoke `33464752484` 进一步暴露目标已发现但再次解析时集合短暂不一致；候选 `944f50c` 把文本、SmartArt 标记和绝对几何编辑改为幂等操作，并只对明确瞬时 HRESULT 或受控的“editable object could not be resolved”错误做最多 8 次退避。生成 PowerShell 回归覆盖瞬时恢复、永久失败不重试、前 8 次空集合第 9 次就绪，以及前三次目标 ID 不一致第四次恢复。

`944f50c` 的 PR CI `33465770051`、[Office smoke 33465770050](https://github.com/cbhandsun/common-tools/actions/runs/33465770050) 及 [full 33466706070](https://github.com/cbhandsun/common-tools/actions/runs/33466706070) 均通过，后者的 corpus 为 31/31、耗时 948877 毫秒，完整核心步骤为 21 分 35 秒。文档提交后的额外 smoke 随后又捕获三个真实冷启动边界：原始文件初次为 clean 时，PowerPoint `SaveCopyAs` 规范化后的副本可能短暂仍为 dirty；保存重开后 `Slides.Count` 可能短暂为 0；完成全部 31/31 后 keeper `Quit` 可能返回瞬时 `0x80010001`。修复分别只对精确的安全字段组合、重开集合暂态，以及 `0x80010001`/`0x8001010A` 做有界重试；真实的原文件修复状态、永久目标错误和永久退出错误仍立即失败。生成脚本回归、本地失败工件单例、本地真实 `mixed-warm.pptx` 和 keeper 三轮真实启停均通过；keeper 仍保留 30 秒进程退出门禁，并且失败诊断只允许输出阶段和 HRESULT。

最终候选 `d054591f` 的 push CI `33474523595`、PR CI `33474528301` 和 [Office smoke 33474528287](https://github.com/cbhandsun/common-tools/actions/runs/33474528287) 均通过；smoke 为 4/4 corpus、4/4 跨渲染器、5/5 独立编辑往返、2/2 批量往返，趋势失败 0。精确提交的 [full 33475598003](https://github.com/cbhandsun/common-tools/actions/runs/33475598003) 也已通过：31/31 corpus、4/4 跨渲染器、5/5 独立编辑往返、2/2 批量往返及 31/31 趋势目标全部通过；30 个会话请求全部接受，释放余量与 stderr 均为 0，旧 11 条历史逐对象原样保留并只追加第 12 条。该轮 corpus 为 1457060 毫秒、完整核心步骤为 32 分 4 秒，仍比 `a8586fa` 基线的 2764956 毫秒和 55 分 33 秒分别减少 47.30% 与 42.27%，但明显慢于前一成功 full，故保留为可观察的 Runner/Office 性能波动，不以最佳单轮冒充稳定下界。脱敏数据、工件 digest 及限制见 [PowerPoint 会话复用证据](./evidence/office-powerpoint-session-reuse-2026-08-31.json)；详细边界见 [Runner 缓存说明](./office-runner-cache.md)。生产部署、真实身份、授权设备/模板及运维验收边界不变。
