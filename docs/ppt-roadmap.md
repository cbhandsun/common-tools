...
| P1 | 独立的新建 PPT 语料 | 进行中 | 覆盖 4 套主题和全部 22 个布局；中文、英文和中英混排；表格、图表、模板、素材、备注、引用；空值、非法值、长内容和容量边界；具备 PowerPoint 与 LibreOffice 证据 | PR #19 将独立 corpus 扩展为 5 份、33 页，在原有 4 套主题、22 个布局、三种语言形态、表格、图表、受控素材、备注、引用和 8 类输入边界基础上，增加一个哈希绑定的用户模板归档；本机 PowerPoint 与 LibreOffice 已通过，待受保护 Office 门禁确认 | PPT 质量验收 | 持续推进 |
| P1 | 三次兼容质量快照 | 已完成 | 相同环境 fingerprint 下连续 3 次通过，并且没有质量回退 | Office run `33330018954` 的脱敏 `quality-history.snapshot.json` 保留同一环境 fingerprint 下 5 个已通过且经趋势门禁保存的连续快照；该次失败运行自身未写入历史 | PPT 质量验收 | 已完成 |
