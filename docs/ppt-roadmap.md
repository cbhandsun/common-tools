...
真实双案例集成中两例均通过，分别约 12.7 秒和 4.3 秒，第二例 COM 附着 36 毫秒、终结器 49 毫秒；统一关闭返回时 POWERPNT 进程数为 0。首轮 full `33457631577` 的 corpus 31/31 通过，但后置独立编辑往返第 5/5 个在关闭时收到瞬时 `0x80010001`；后续 smoke `33460946205` 又暴露首个 deck 的可编辑集合在原 8 次等待窗口内未就绪。修复只对两个明确的瞬时 RPC HRESULT 做最多 5 次关闭退避，并把只读目标发现窗口扩为 12 次；永久错误仍立即失败。文档候选 `8c53768` 的 smoke `33464752484` 进一步暴露目标已发现但再次解析时集合短暂不一致；最终修复把文本、SmartArt 标记和绝对几何编辑改为幂等操作，并只对明确瞬时 HRESULT 或受控的“editable object could not be resolved”错误做最多 8 次退避。生成 PowerShell 回归覆盖瞬时恢复、永久失败不重试、前 8 次空集合第 9 次就绪，以及前三次目标 ID 不一致第四次恢复。

最终候选 `944f50c` 的 PR CI `33465770051` 和 [Office smoke 33465770050](https://github.com/cbhandsun/common-tools/actions/runs/33465770050) 均通过；同提交的 push CI `33465767591` 只在无关的 Windows `worker doctor` 外部进程测试达到既有 15 秒时限，PR CI 独立复跑该门禁通过，未提高时限或跳过测试。[full 33466706070](https://github.com/cbhandsun/common-tools/actions/runs/33466706070) 为 31/31 语料、4/4 跨渲染器、5/5 独立编辑往返、2/2 批量往返及 31/31 趋势目标通过，失败数为 0。会话适用 30 例、请求 30、拒绝 0、释放余量 0、stderr 0；历史旧 10 条逐对象原样保留并只追加第 11 条。corpus 从基线 2764956 毫秒降至 948877 毫秒，减少 65.68%；完整核心步骤从 55 分 33 秒降至 21 分 35 秒，减少 61.15%。脱敏数据、工件 digest 及限制见 [PowerPoint 会话复用证据](./evidence/office-powerpoint-session-reuse-2026-08-31.json)；详细边界见 [Runner 缓存说明](./office-runner-cache.md)。生产部署、真实身份、授权设备/模板及运维验收边界不变。
