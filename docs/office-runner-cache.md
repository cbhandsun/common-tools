...
本机缓存新增回归覆盖冷/热复用、工作区内依赖、真实离线 npm 检查、分支隔离、无效输入/快照、越界与链接、容量边界和复制失败，统一单元测试入口自动发现 `test/office-node-local-cache.test.js`。当前仅完成本地验证；专用 Runner 的冷/热运行耗时与完整 Office 验收仍待采集，不能把下面旧版本的耗时当成本机缓存收益。
