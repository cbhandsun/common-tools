...
文件复制最多允许 8 个在途操作。出现失败后停止派发新文件，等待已开始的复制全部结束，再进行 staging 清理并报告失败，避免清理与尚未结束的写入互相竞争；目录和来源边界仍在开始复制前统一校验。并发上限、字节保留、停止派发和等待在途复制的行为均有回归测试。该优化的真实 Runner 耗时仍需单独测量，不能仅凭使用并发就宣称提速。
...
本机缓存新增回归覆盖冷/热复用、工作区内依赖、真实离线 npm 检查、分支隔离、无效输入/快照、越界与链接、容量边界和复制失败，统一单元测试入口自动发现 `test/office-node-local-cache.test.js`。当前受控并发复制版本仅完成本地验证；该版本专用 Runner 的耗时与完整 Office 验收仍待采集，不能把下面旧版本的耗时当作并发复制收益。
...
后续受控并发版本的本机真实依赖快照保存测量约 12.5 秒，保存后枚举校验共约 16.4 秒，10,380 个条目约 72.4 MB。额外的一次性完整性检查逐文件比较了 9,133 个依赖文件的 SHA-256，全部与来源一致；这项完整哈希检查不加入每轮准备路径。以上只证明本机保存路径，不包含 Runner 恢复、校验或整条 CI；专用 Runner 的并发版本测量仍待完成。
