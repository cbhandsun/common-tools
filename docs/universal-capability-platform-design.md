...
首版仅删除未被任何 OOXML relationship 引用的 `ppt/media/*` 孤儿媒体，绝不编辑文本、布局、样式或可见对象，也不会覆盖输入文件。本地同步命令为 `common-tools ppt-improve run --input <deck.pptx> --report <ppt-quality-report.json> --out <improve-dir>`；需要异步控制时可先 `create` 再运行 Job。若需要完整本地工作流，`common-tools ppt-improve pipeline --input <deck.pptx> --out <new-pipeline-dir>` 会先生成 `quality/ppt-quality-report.json/.md`，只在审查 Job 成功后以该报告创建 `improve/` Job；输出根必须此前不存在，避免覆盖既有审核或改善工件。MCP 对应 `create_ppt_improve_job`、`get_ppt_improve_report`。可选团队 Worker 采用单一 PPTX 输入，但会在受限临时目录内自行生成与输入 SHA-256 绑定的初审报告，随后才做相同的安全改善；因此不会接受调用方提供的报告、修复脚本或路径。有安全候选时除 `improved.pptx` 和改善报告外，还会生成与新副本 SHA-256 绑定的 `improved-ppt-quality-report.json/.md`，使复审成为可交付工件；所有目标名采用排他创建，若输出目录已有任一同名文件，整个 Job 会失败并只回滚本次已创建的文件。没有安全候选时只生成独立改善报告，不伪造新的 PPTX 或复审。视觉、版式、文案和可编辑性修复仍应作为后续受控 repair profiles，产出后必须再次走 `ppt-quality`。
