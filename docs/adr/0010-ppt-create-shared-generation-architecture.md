...
Document ingestion preserves more than flat text: DOCX tables become native table visuals and PDF form-feed boundaries create page-aware heading structure. Safe PPTX templates are inspected into a bounded semantic layout map containing names, placeholder types, capacity, and applicable roles; mapped layout identity is recorded per Deck IR page. Direct IR editing is revision-bound and supports snapped geometry, validated style batches, collection-local layer ordering, and bounded add-text/duplicate/delete object lifecycle operations. The offline IR preview denies network connections with a nonce-bound content security policy and refuses empty, oversized, or over-count patch downloads before server-side validation.

The image-to-editable local path also accepts an explicitly ordered batch of 2–20 unique images from one workspace directory. The Job persists that order, the runner forwards one bounded input argument per page, and the OpenXML regression asserts the final page count. The controlled IR editor exposes whole-table and chart-data panels without accepting raw chart JSON. A scheduled Windows Office gate opens, edits, saves, and revalidates the generated semantic deck and the two-page image batch deck.
