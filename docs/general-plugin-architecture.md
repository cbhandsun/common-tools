...
| 评估项 | 当前状态 | 判断 |
| --- | --- | --- |
| 能力发现与门控 | `packages/capability-registry` 聚合 capability module，统一暴露本地能力、工具 handler、报告 reader 与 UI contribution；registry 加载时会对齐签名 manifest 的 capability、toolNames、runtime range 和 worker profile；MCP 层消费注册结果 | 合理 |
...
3. UI contribution 已经通过 registry 聚合，但 capability module 仍在 registry 内手写 require；后续可让每个 capability package 直接导出标准 module，使 registry 只负责发现与校验。
