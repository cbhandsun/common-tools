...
function pptCreateLayoutProbe() {
  return [
    ...
    "const version=String(manifest.version||'').split('.').map(Number);",
    "stage='registry-load';const themes=require(path.join(root,'packages','ppt-create-core','theme-registry.js'));const layouts=require(path.join(root,'packages','ppt-create-core','layout-registry.js'));const models=require(path.join(root,'packages','ppt-create-core','data-models.js'));const editor=require(path.join(root,'packages','ppt-create-core','editor.js'));const exporter=require(path.join(root,'packages','ppt-create-core','export.js'));const pdf=require(path.join(root,'packages','ppt-create-core','libreoffice-pdf.js'));const planner=require(path.join(root,'packages','ppt-create-core','layout.js'));const schema=require(path.join(root,'packages','ppt-create-core','presentation-spec.schema.json'));",
