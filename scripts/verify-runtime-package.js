...
function imageEditableEnhancementProbe() {
  return [
    ...
    "const worker=require(path.join(root,'packages','slideclone-core','team-worker.js'));const archive=require(path.join(root,'packages','slideclone-core','team-raw-image-archive.js'));const quality=require(path.join(root,'packages','slideclone-core','team-render-quality.js'));",
    ...
    "const checks=[['capability-version',versionReady],['native-export',typeof native.residualEraseObjects==='function'],['worker-export',typeof worker.residualDeduplicationStatus==='function'],['batch-archive-export',typeof archive.createRawImageArchive==='function'&&archive.MAX_RAW_IMAGE_ARCHIVE_PAGES===20],['batch-quality-export',typeof quality.createRawImageRenderQualityVerifier==='function'],['local-batch-core',localCoreSource.includes('MAX_EDITABLE_BATCH_INPUTS')&&localCoreSource.includes('inputPaths')],['local-batch-cli',cliSource.includes('editable batch')&&cliSource.includes('--inputs')],['local-batch-skill',localSkill.includes('common-tools editable batch')&&localSkill.includes('200,000,000 decoded pixels')],['residual-export',typeof residual.eraseObjectMask==='function'],['remote-worker-wiring',remoteSource.includes('createNativeRebuilder')&&remoteSource.includes('full-slide-native-residual')],['native-strategy',nativeSource.includes('full-slide-object-erased-residual')],['worker-quality-gate',workerSource.includes('residual-native-duplicates-removed')],['batch-worker-contract',workerSource.includes('raw-image-batch-validated')&&workerSource.includes('MAX_RAW_IMAGE_PAGES')],['marketplace-skill-gate',skill.includes('residual-native-duplicates-removed')&&skill.includes('raw-image-batch-validated')]];",
