...
// Bump only when an installed Codex OAuth session must be re-established.
// Keeping this independent from the plugin version avoids prompting on routine upgrades.
const CODEX_MCP_SERVER_NAME = "common-tools-auth-v2";
const LEGACY_CODEX_MCP_SERVER_NAMES = Object.freeze(["common-tools"]);
...
function pluginMetadata(host, name = pluginName(), capabilities = CAPABILITIES) {
  ...
  return {
    ...
    interface: {
      ...
      defaultPrompt: [capabilities.includes("project-audit") ? "Ask me to choose a numbered, Chinese, or English project-audit scope before executing the approved audit." : "Use the installed common-tools capability to process this approved input."]
    }
  };
}
...
function installGuide(host, origin, capabilities, layout = "bundle") {
  ...
  const codexMcpNote = host === "codex"
    ? `For Codex, use the included installer rather than only running the two marketplace commands: before changing the client, it verifies the remote \`/readyz\` endpoint and that the selected capability scopes are advertised. It then reconciles the global \`${CODEX_MCP_SERVER_NAME}\` HTTP MCP server at \`${origin}/mcp\`, safely removes a recognized legacy Common Tools registration, binds the public OAuth client \`common-tools-mcp\`, and starts the browser login only when authorization is required. This avoids relying on host-specific plugin MCP auto-registration.\n\n`
    : "";
...
function remoteSkill(capability, origin) {
  ...
  const serviceOrigin = parseOrigin(origin);
...
For gates, require explicit authorization and run \`common-tools audit run --mode gates --scope <selected-scope-ids> --run-gates --out .common-tools/reports/project-audit\`. For experience, obtain explicit permission before browser work, create an eight-scenario manifest with \`common-tools audit evidence-template --out audit-evidence/experience.json\`, translate the approved natural-language journey into a bounded local plan, then use \`common-tools audit experience-collect --plan audit-evidence/plan.json --out audit-evidence/capture --run-browser\` only after the user starts the local app. The collector allows loopback targets and fixed safe actions, blocks off-origin HTTP(S) requests by default, and writes only local screenshot plus aggregate console/network evidence. Collection proves capture completion only; scenarios remain \`not-verified\` until every screenshot and console/network artifact is inspected. Reject blank, loading, blocked, cropped, wrong-state, and error-page screenshots. Do not infer keyboard, focus, contrast, reflow, screen-reader, or recovery health from screenshots alone. Pass a separately reviewed manifest as \`common-tools audit run --mode experience --scope <selected-scope-ids> --experience-evidence <reviewed-manifest.json> --out .common-tools/reports/project-experience\`; require both \`--scope <selected-scope-ids>\` and \`--run-gates\` for full review. Do not silently downgrade requested experience/full review to static-only.
...
For Codex, an upgrade changes the managed OAuth connection to \`${CODEX_MCP_SERVER_NAME}\` only when the authentication contract requires a fresh login. If the requested remote tools are absent and local command execution is available, inspect that connection without reading credentials, require its URL to equal the packaged HTTPS MCP URL, and let the agent repair a missing registration. If authorization is unavailable, let the agent reset only that managed connection and initiate browser login with \`offline_access\` plus only the requested capability scope. The user should complete the browser login but must not be asked to copy or execute a script or command. After successful authorization, require a full Codex restart and a new task before deciding whether tools are loaded.
...
function mcpConfiguration(host, origin, serverName) {
  const server = { type: "http", url: `${origin}/mcp` };
  if (!['codex', 'claude'].includes(host)) throw new TypeError("plugin host is invalid");
  const resolvedServerName = serverName || (host === "codex" ? CODEX_MCP_SERVER_NAME : "common-tools");
  return { mcpServers: { [resolvedServerName]: { ...server, oauth: { clientId: "common-tools-mcp" } } } };
}
...
function chineseCapabilityGuide(capability) {
  ...
  return [
    ...
    capability === "siyuan-note"
      ...
      ? "在 Codex 或 Claude 中直接说明审计目标。执行前必须先选择审计范围：1、all 或全部四域代表全部四域，且只能单独使用；也可用 2,3、product-journey,engineering-delivery、产品闭环，工程与交付这类中英文组合选择产品闭环、视觉交互、数据/权限/可靠性、工程交付；不要把普通审计请求静默当作全域。普通“审计当前项目”会走 `enhanced` 本机只读审视；“只做代码审计”才走 `code`。若已安装 Local Runtime，先运行 `common-tools runtime resolve --capability project-audit`：只有用户明确要求团队/远程审计且策略允许时才上传归档。远程作业也必须传入同一解析后的 `options.auditScope`。未安装 Local Runtime 时，不要把普通请求自动改为远程上传。"
...
function writePluginPackage(host, origin, hostRoot, details) {
  ...
  if (routerSkill) {
    ...
    for (const capability of capabilities) {
      ...
      fs.writeFileSync(path.join(capabilitySkillRoot, "SKILL.md"), remoteSkill(capability, origin), "utf8");
    }
  } else {
    for (const capability of capabilities) {
      const skillRoot = path.join(pluginRoot, "skills", capability);
      fs.mkdirSync(skillRoot, { recursive: true });
      fs.writeFileSync(path.join(skillRoot, "SKILL.md"), remoteSkill(capability, origin), "utf8");
    }
  }
...
function writePlugin(host, origin, output, capabilities, layout, dependencies = {}) {
  ...
  const plugins = layout === "split"
    ? capabilities.map((capability) => ({ name: pluginName(capability), capabilities: [capability], serverName: host === "codex" ? `${CODEX_MCP_SERVER_NAME}-${capability}` : `common-tools-${capability}` }))
    : [{ name: pluginName(), capabilities, serverName: host === "codex" ? CODEX_MCP_SERVER_NAME : "common-tools", routerSkill: true }];
...
module.exports = { CAPABILITIES, CODEX_MCP_SERVER_NAME, LEGACY_CODEX_MCP_SERVER_NAMES, LOCAL_RUNTIME_CAPABILITIES, LOCAL_RUNTIME_VERSION, REMOTE_CAPABILITY_CODES, REMOTE_CAPABILITY_GUIDANCE, REMOTE_CAPABILITY_SCOPES, REMOTE_PLUGIN_VERSION, connectionVerificationScript, generateRemotePluginBundles, installGuide, installationScript, localRuntimeCopyFilter, localRuntimeInstaller, marketplaceMetadata, mcpConfiguration, parseArguments, parseCapabilities, parseLayout, parseOrigin, pluginName, remoteRouterSkill, remoteSkill, writeLocalRuntimePayload };
