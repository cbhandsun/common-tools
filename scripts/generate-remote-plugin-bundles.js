...
开始操作前，先确认当前会话至少能看到 \`siyuan_list_notebooks\`。如果一个 \`siyuan_*\` 工具都没有，不要推断思源服务端故障，也不要改用其他笔记服务；应说明当前任务没有加载 Common Tools MCP 授权工具。Codex 用户应在安装插件的电脑上运行 \`codex mcp get common-tools --json\`、\`codex mcp logout common-tools\` 和 \`codex mcp login common-tools --scopes offline_access,common-tools:capability:siyuan-note\`。如果配置不存在，先运行 \`codex mcp add common-tools --url ${serviceOrigin}/mcp --oauth-client-id common-tools-mcp\`。浏览器授权后必须完全关闭并重新打开 Codex，再新建任务；只有新任务能看到 \`siyuan_list_notebooks\` 后才能继续。禁止要求用户提供思源 Token，禁止在工具缺失时声称已经恢复。
